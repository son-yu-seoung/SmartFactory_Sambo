...
cap = cv2.VideoCapture(0, cv2.CAP_DSHOW) # 종료할 때 [WARN:0]오류 제거 
...
while(True):
    ...
    frame =cv2.resize(frame, (416, 416), interpolation=cv2.INTER_LINEAR) # 비디오 영상의 크기 설정
    ...
    if key == ord('q') or key == ord('Q'):
        ...
        for i in range(10): # 10장의 사진을 찍는다.
